...
def hook_sift_layer(model, hidden_size, learning_rate=1e-4, init_perturbation=1e-2,
                    target_module='cond_rnn'): #'embeddings.LayerNorm'
    """
...
class AdversarialLearner:
    ...
    def loss(self, target, logits_fn, loss_fn='symmetric-kl', *wargs, **kwargs):
        ...
        pert_loss = loss_fn(pert_logits, target.detach()).sum().requires_grad_()#; print (pert_loss)
        pert_loss.backward()
...
def bce(logits, target):
    ...
    return bceloss(logit_stu.view(-1), logit_tea.view(-1))

def smoothl1(logits, target):
    logit_stu = logits.view(-1, logits.size(-1)).float()
    logit_tea = target.view(-1, target.size(-1)).float()
    return F.smooth_l1_loss(logit_stu.view(-1), logit_tea.view(-1))
