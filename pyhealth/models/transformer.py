...
class Transformer(BaseModel):
    ...
    def search_drug(self,
        ...
        """
        vectors = [i['drug'][0].clone().detach() for i in mattching_list[seq]]
    ...
    def second_stage(self,
        ...
        return tmp_mattching_mean_icudays, day_reward
    ...
    def forward(self, **kwargs) -> Dict[str, torch.Tensor]:
        ...
        real_icudays = sum(kwargs['readmission'])/len(kwargs['readmission'])
        icd_result = kwargs["matching"]
        ...
        # save time
        if self.training:
            mattching_mean_icudays, _ = self.second_stage(y_prob, icd_result)
            init_mean_icudays = real_icudays
        else:
            mattching_mean_icudays, _ = self.second_stage(y_prob, icd_result)
            init_mean_icudays, _ = self.second_stage(y_true, icd_result)
            #_, mattched_days = self.second_stage(y_prob, icd_result)

        return {"loss": loss,
                "y_prob": y_prob,
                "y_true": y_true,
                "real_icudays": real_icudays,
                "init_mean_icudays": init_mean_icudays,
                "mattching_mean_icudays": mattching_mean_icudays,
                #"real_icudays2": kwargs['readmission'],
                #"mattched_days": mattched_days
                }
