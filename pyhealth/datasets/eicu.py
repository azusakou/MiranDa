...
class drugname2ndc11:
    def __init__(self):
        # Load the drug-ndc-0001-of-0001.json file
        with open('/home/user/Documents/CODE/MIMIC/PyHealth/input/drug-ndc-0001-of-0001.json') as f:
            self.data = json.load(f)

    def convert_to_11digit(self,ndc):
        ndc_parts = ndc.split("-")
        if len(ndc_parts[0]) == 4:
            ndc_parts[0] = "0" + ndc_parts[0]
        if len(ndc_parts[1]) == 3:
            ndc_parts[1] = "0" + ndc_parts[1]
        if len(ndc_parts[2]) == 1:
            ndc_parts[2] = "0" + ndc_parts[2]
        return "-".join([ndc_parts[0], ndc_parts[1], ndc_parts[2]])

    # Define a function to search for the NDC code based on the drug name

    def search_drug(self, drug_name):
        for result in self.data['results']:
            try:
                if drug_name.lower() in result['generic_name'].lower():
                    if len(result['packaging']) == 0:
                        return "0"
                    else:
                        return self.convert_to_11digit(result['packaging'][0]['package_ndc']).replace('-','')
            except KeyError:
                pass
        return "0"
...
class eICUDataset(BaseDataset):
    ...
    def parse_basic_info(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        ...
        patient_df = pd.read_csv(
            ...
            nrows=10000 if self.dev else None,
        )
        patient_df = patient_df.drop(patient_df.loc[patient_df['unitdischargelocation'].isin(['Other Hospital', 'Other ICU'])].index)
        # read hospital table
    ...
    def parse_medication(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        ...
        df = pd.read_csv(
            os.path.join(self.root, f"{table}.csv"),
            low_memory=False,
            dtype={"patientunitstayid": str, "ndcdrugname": str},
        )
        # drop rows with missing values
        df = df.dropna(subset=["patientunitstayid", "ndcdrugname"])
        ...
        for v_id, v_info in tqdm(df_group, desc=f"Parsing {table}"):
            if v_id not in self.visit_id_to_patient_id:
                continue
            patient_id = self.visit_id_to_patient_id[v_id]
            for offset, code in zip(v_info["drugstartoffset"], v_info["ndcdrugname"]):
                timestamp = patients[patient_id].get_visit_by_id(
                    v_id
                ).encounter_time + pd.Timedelta(minutes=offset)
                event = Event(
    ...
    def parse_lab(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        ...
        df["labname"] = df["labname"] + df["labresulttext"]
