    ...
    def stat(self) -> str:
        ...
        for key in self.samples[0]:
            ...
            lines.append(
                ...
                f" std {np.std(num_events):.4f}"
                f"95% CI upper {(sum(num_events) / len(num_events)) + np.std(num_events)/np.sqrt(len(num_events))*1.96} "
                f"95% CI lower {(sum(num_events) / len(num_events)) - np.std(num_events) / np.sqrt(len(num_events)) * 1.96} "
            )
