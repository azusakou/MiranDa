...
class MIMIC3Dataset(BaseDataset):
    ...
    def parse_prescriptions(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        ...
        # drop rows with missing values
        df.drop(index=df[df["NDC"]=="0"].index, axis=0,inplace=True) # TODO
    ...
    # TODO
    def parse_text(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        """Helper function which parses PRESCRIPTIONS table.
        Will be called in `self.parse_tables()`
        Docs:
            - PRESCRIPTIONS: https://mimic.mit.edu/docs/iii/tables/prescriptions/
        ...
        df = pd.read_csv(
            os.path.join(self.root, f"{table}.csv"),
            low_memory=False,
            dtype={"SUBJECT_ID": str, "HADM_ID": str, "TEXT": str},
        )
        # drop rows with missing values
        df = df.dropna(subset=["SUBJECT_ID", "HADM_ID", "TEXT"])
        # sort by start date and end date
        df = df.sort_values(
            ["SUBJECT_ID", "HADM_ID", ], ascending=True
        )
        # group by patient and visit
        group_df = df.groupby(["SUBJECT_ID", "HADM_ID"])
        # iterate over each patient and visit
        for (p_id, v_id), v_info in tqdm(group_df, desc=f"Parsing {table}"):
            for code in v_info["TEXT"]:
                event = Event(
                    code=code,
                    table=table,
                    vocabulary="TEXT",
    ...
    def parse_labevents(self, patients: Dict[str, Patient]) -> Dict[str, Patient]:
        ...
        df["FLAG"] = df["FLAG"].replace({"abnormal":'.9'})
        df["FLAG"] = df["FLAG"].fillna('.1')
        df["ITEMID"] = df["ITEMID"] + df["FLAG"]
        # sort by charttime
